                                ...
                                @foreach ($allSupplier as $data )
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <a href="#" id="edit_supplier" data-id="{{ $data->id }}" class="btn btn-sm btn-success p-1 px-2" style="margin-right:-13px">
                                                <i class="fa fa-pencil"></i><span class="btn-icon-add"></span>Edit
                                            </a>
                                            <a href="#" id="view_supplier" data-id="{{ $data->id }}" class="btn btn-sm btn-danger p-1 px-2" style="margin-left: 19px">
                                                <i class="fa fa-pencil"></i><span class="btn-icon-add"></span>View
                                            </a>
                                            <a href="#" id="delete_supplier" data-id="{{ $data->id }}" class="btn btn-sm btn-info p-1 px-2" style="float: right">
    ...
    $(document).on('click', '#edit_supplier', function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                $('#type').val(response.type === 'Distributor' ? 'Distributor' : response.type === 'Whole Seller' ? 'Whole Seller' : 'Broker').prop('selected', true);
                $('#remarks').empty();
                ...
                $('#imagePreview').show();
                var updateRoute = '{{ route('update_supplier', ['id' => ':id']) }}';
                updateRoute = updateRoute.replace(':id', response.id);
                $('#supplierForm').attr('action', updateRoute);
                $('#supplierForm').find('button[type="submit"]').text('Update');
            }
        });
    });
    ...
    $(document).on('click', '#view_supplier', function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                console.log(response);
                $('#sup_name').empty();
                $('#sup_name').val(response.name).prop('readonly', true);
                $('#email').empty();
                ...
                $('#type').val(response.type === 'Distributor' ? 'Distributor' : response.type === 'Whole Seller' ? 'Whole Seller' : 'Broker').prop({'selected': true,'disabled': true});
                $('#accNumber').empty();
                ...
                $('#supplierForm').find('button[type="submit"]').hide();
            }
        });
    });
